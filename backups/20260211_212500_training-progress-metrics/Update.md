...
- `src/mock/data.js`
  - 移除 Body Fat 與 VO2 max 資料與 series。
  - 調整為只輸出：
    - Squat (3RM / 5RM / PR=1RM)。
    - Body Weight（kg）。
  - 將 Body Weight 的 `yAxis` 調整為與 Squat 相同的主軸 (`yAxis: 0`)，統一使用 kg 單位。

- `src/views/DashboardView.vue`
  - 將第一張圖表的 `HistoryChart` 從 `:dualAxis="true"` 改為單一 Y 軸（移除 `dualAxis` 設定），畫面上不再出現左右兩側不同刻度。
...
3. 不再出現 Body Fat 與 VO2 max 線條或圖例。
