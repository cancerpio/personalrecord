...
// 固定的 Squat / 體重 / 體脂 / VO2 max 資料（4 週）
const SQUAT_DATA = {
    '5RM': [90, 90, 100, 105],
    '3RM': [100, 105, 110, 110],
    // PR 對應使用 1RM 資料
...
const BODY_FAT_DATA = [23, 24, 21, 20];
const VO2MAX_DATA = [40, 40, 39, 39];
        ...
        setTimeout(() => {
            ...
            const relativeSeries = [{
                ...
                data: timestamps.map((t, index) => {
                    ...
                    if (!bw) return null;
                    const squat = squatValues[index];
                    const ratio = squat / bw;
                    return {
                        x: t,
            ...
            resolve({
                series: [squatSeries, bodyweightSeries, bodyFatSeries, vo2MaxSeries],
                relativeSeries,
                sparklines,
                cycleWeeks
            });
