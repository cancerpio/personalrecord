...
日期控制項 SHALL 提供**前後一天的快速切換**。切換 SHALL 以日曆日為單位，
且 SHALL NOT 取代直接選擇日期的方式——快速切換為捷徑，跨度較大的跳轉仍由日期選擇器承擔。

**日期 SHALL NOT 被切換至今天之後。** 理由是快速切換讓連點兩下就能把日期帶到明天，
而錯誤日期的紀錄事後極難察覺；現階段亦無記錄未來日期的情境。
處於今天時「下一天」SHALL 呈停用而非隱藏——隱藏會使控制項位置變動。
此限制 SHALL 同時在互動層與行為層各擋一次，SHALL NOT 只依賴按鈕的停用狀態。

（此為**現階段的約束**，SHALL NOT 解讀為「本能力不涉及課表規劃」的產品定位宣示。
已知未來變更條件：若本專案導入「預先安排課表」，屆時應在資料模型上區分
「計畫」與「已執行」，而非單純放行未來日期。）
...
#### Scenario: 快速切換至前一天
- **WHEN** 使用者於日期控制項選擇「前一天」
- **THEN** 所選日期 SHALL 為原日期的前一個日曆日，三個區塊 SHALL 一併更新

#### Scenario: 今天時不得往未來切換
- **WHEN** 所選日期為今天
- **THEN** 「下一天」SHALL 呈停用狀態，且 SHALL NOT 因任何觸發方式而切換至明天
