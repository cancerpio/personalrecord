...
容器 SHALL 同時提供**返回本列表**的方式。選定動作會將使用者捲離列表，
而該捲動是由選取行為引入的，因此返回路徑屬於本需求的一部分，
SHALL NOT 只提供單向的前往。

本能力 SHALL NOT 在列表中標示「目前選取」的列——圖表區塊為常駐顯示、
永遠反映某一個動作，選取不構成一種可退出的模式，標示反而會暗示存在模式。
