...
用字 SHALL 為「持續」而非「連續」：本指標容許每次連空一週，
其問題是「現在還持續著嗎」，SHALL NOT 讀作嚴格連續。

單位 SHALL 出現在欄標題，SHALL NOT 只寫在區塊說明文字裡——
說明文字是需要記住的圖例，欄標題就在該欄正上方。
區塊說明文字則 SHALL 說明持續週數的**語意**（同一動作連續幾週沒換過）：
單位解決「這個數字的量綱是什麼」，語意解決「這個數字在回答什麼問題」，
兩者 SHALL NOT 互相取代。

最重欄的標題 SHALL 為「最重」，SHALL NOT 標示為「PR」或「1RM」——
該欄為不限 reps 的最大重量（見 All-Time Max Weight），與 1RM 不是同一個量。

區塊標題含有的時間範圍 SHALL NOT 被理解為限制任何欄位的計算範圍。
最重欄 SHALL 維持掃描全部歷史，動作列入條件 SHALL 維持不限時間範圍——
標題與欄位範圍不一致時，SHALL 修正標題，SHALL NOT 縮減欄位的計算範圍。

持續週數 SHALL 以文字顏色分段呈現：4 週以上為警示色、3 週為琥珀色、
其餘為一般文字色。SHALL NOT 使用 pill、emoji、警示 icon 等警報語彙——
本能力不通知，使用警報符號卻無後續行動在語意上自相矛盾。

顏色門檻為視覺分段而非警報門檻，SHALL 可在不影響正確性的前提下調整。

#### Scenario: 高持續週數以警示色顯示
- **WHEN** 某動作持續週數為 6
- **THEN** 該數字 SHALL 以警示色顯示，且 SHALL NOT 伴隨任何警示 icon 或通知

### Requirement: Row Selection
每一**資料列** SHALL 可點擊。點擊時本能力 SHALL 發出「切換該動作」的訊號，
並 SHALL NOT 自行處理導覽或任何狀態變更——本能力維持純表現層，
後續行為由容器決定。**表頭列 SHALL NOT 可點擊。**

容器收到訊號時 SHALL 於該列下方展開該動作的詳細面板（見 `exercise-detail-panel`），
SHALL NOT 捲動至頁面其他位置。因為不發生捲動，本能力 SHALL NOT 需要提供
「返回列表」的路徑——沒有東西把使用者帶離列表。

展開中的列 SHALL 以展開／收合指示符號標示其狀態。此處使用指示符號
**不違反** Presentation Without Alerting：該原則禁止的是「外觀像可操作元件卻不可點」
的假暗示，而此符號對應一個真實存在、可雙向操作的行為。

本能力 SHALL NOT 使用 pill、emoji 或警示 icon 表達選取狀態——
展開狀態不是警報，其顯著性由指示符號與展開的內容本身承擔。

#### Scenario: 點擊資料列展開詳細面板
- **WHEN** 使用者點擊某一收合中的資料列
- **THEN** 該動作的詳細面板 SHALL 展開於該列下方，畫面 SHALL NOT 捲動至其他區塊
...
#### Scenario: 展開狀態有明確指示
- **WHEN** 某一列的詳細面板為展開狀態
- **THEN** 該列 SHALL 顯示對應的展開指示符號，收合時 SHALL 顯示收合指示符號
